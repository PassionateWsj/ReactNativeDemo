...
export default class Util {
    ...
    //判断数组是否包含指定元素
    static containsUUID(arr, obj) {
        var i = arr.length;
        while (i--) {
            if (arr[i].uuid && arr[i].uuid === obj) {
                return true;
            }
        }
        return false;
    ...
    static isIphoneX() {
        return (
            ...
            ((deviceHeight === XSM_HEIGHT && deviceWidth === XSM_WIDTH) ||
                (deviceHeight === XSM_WIDTH && deviceWidth === XSM_HEIGHT))
        );
    }
     ...
     * @returns {number}
     */
    static calculateCharLength(str) {
